...
// NewGoodiesClient creates an instance of client to goodies server
// protocol accepts two values: http/tcp
// address specifies the address of a server to connect to:
// e.g. for http instance: "http://server:9006"
// for tcp instance: "server:19006""
func NewGoodiesClient(protocol string, address string) Provider {
	switch protocol {
	case "http":
		return goodiesClient{NewGoodiesHttpCommandClient(address)}
	case "tcp":
		return goodiesClient{NewTCPBasedClient(address)}
	}
	panic("Unknown client type requested: " + protocol)
}

func internalProcess(req CommandRequest, c goodiesClient) CommandResponse {
	res := c.transport.Process(req)
	return res
}
...
func (c goodiesClient) ListRemoveValue(key string, value string) error {
	req := CommandRequest{"ListRemoveValue", []string{key, value}}
	res := internalProcess(req, c)
	if !res.Success {
		return res.Err
	}
